 ...
 */
@SourceType(value = "GBASE", label = "GBase")
public class GBaseConf extends AbstractArpConf<GBaseConf> {

    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(GBaseConf.class);

    private static final String ARP_FILENAME = "arp/implementation/gbase-arp.yaml";
...
//        logger.info(String.format("JDBC URL ---- jdbc:gbase://%s:%s/%s", host, port, database));
//        return String.format("jdbc:gbase://%s:%s/%s", host, port, database);
        logger.info(String.format("JDBC URL ---- jdbc:gbase://%s:%s", host, port));
        return String.format("jdbc:gbase://%s:%s", host, port);
    }
